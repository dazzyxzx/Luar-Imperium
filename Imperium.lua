...
CMDs[#CMDs + 1] = {NAME = 'serverfinder/sf/findserver/fsv', DESC = 'Server Finder.'}
...
local SFStats = game:GetService("Stats")
local SFTween = TweenService
local SFRun = RunService
local SFUserInput = UserInputService
...
local SFRegions = {
    {name = "Indonesia", off = 0},
    {name = "Singapore", off = 25},
    {name = "Malaysia", off = 30},
    {name = "Thailand", off = 40},
    {name = "Philippines", off = 35},
    {name = "Vietnam", off = 45},
    {name = "China", off = 55},
    {name = "Japan", off = 70},
    {name = "South Korea", off = 80},
    {name = "Hong Kong", off = 50},
    {name = "Taiwan", off = 55},
    {name = "Australia", off = 120},
    {name = "India", off = 100},
    {name = "United Arab Emirates", off = 150},
    {name = "United Kingdom", off = 210},
    {name = "Germany", off = 220},
    {name = "France", off = 225},
    {name = "Poland", off = 230},
    {name = "United States", off = 170},
    {name = "Brazil", off = 270},
    {name = "Canada", off = 175},
}

local function SFGetPing()
    local p = 50
    pcall(function()
        local v = SFStats.Network.ServerStatsItem["Data Ping"]:GetValue()
        if v then p = math.floor(v) end
    end)
    return p
end

local function SFRenderRegions(filterText)
    local scroll = SFState.ScrollF
    if not scroll then return end
    for _, b in pairs(SFState.RegionButtons) do
        if b and b.Parent then b:Destroy() end
    end
    SFState.RegionButtons = {}
    filterText = string.lower(filterText or "")
    local pingBase = SFGetPing()
    local count = 0
    for _, data in ipairs(SFRegions) do
        local calcPing = math.max(5, pingBase + data.off)
        local colorTag = "[G] "
        if calcPing >= 200 then colorTag = "[R] "
        elseif calcPing >= 100 then colorTag = "[Y] " end
        if filterText == "" or string.find(string.lower(data.name), filterText) then
            count = count + 1
            ...
            btn.Text = "  " .. data.name .. "  |  " .. colorTag .. calcPing .. "ms"
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                btn.BackgroundColor3 = Color3.fromRGB(30, 110, 60)
                SFState.Selected = data.name
                if SFState.StatusLbl then
                    SFState.StatusLbl.Text = " Selected: " .. data.name .. " (" .. calcPing .. "ms)"
                end
            end)
            table.insert(SFState.RegionButtons, btn)
        end
    end
    scroll.CanvasSize = UDim2.new(0, 0, 0, count * 35)
end

local function SFHop()
    if SFState.Selected == "" then
        if SFState.StatusLbl then
            SFState.StatusLbl.Text = " [!] Please select a country first!"
        end
        return
    end
    if SFState.StatusLbl then
        SFState.StatusLbl.Text = " [*] Fetching servers for " .. SFState.Selected .. "..."
    end
    ...
    if ok and result and result.data then
        ...
        if SFState.PopMode == "Low" then
            table.sort(servers, function(a, b) return a.playing < b.playing end)
        elseif SFState.PopMode == "High" then
            table.sort(servers, function(a, b) return a.playing > b.playing end)
        elseif SFState.PopMode == "New" then
        ...
        for _, s in ipairs(servers) do
            if s.id ~= game.JobId and s.playing < s.maxPlayers and s.playing > 0 then
                ...
                if SFState.Mic == "All" then
                    table.insert(valid, s)
                elseif SFState.Mic == "Voice" and sVoice then
                    table.insert(valid, s)
                elseif SFState.Mic == "Regular" and not sVoice then
                    table.insert(valid, s)
                end
            end
        end
        ...
        if #valid > 0 then
            local target = valid[math.random(1, math.min(5, #valid))].id
            if SFState.StatusLbl then
                SFState.StatusLbl.Text = " [SUCCESS] Connecting..."
            end
            task.wait(0.5)
    ...
    if SFState.StatusLbl then
        SFState.StatusLbl.Text = " [*] Executing fallback server hop..."
    end
    TeleportService:Teleport(game.PlaceId, LocalPlayer)
end

local function SFBuildUI()
    if SFState.Built then return end
    SFState.Built = true

    local f = Instance.new("Frame")
    f.Name = "SF_Panel"
    f.Size = UDim2.new(0, 350, 0, 370)
    f.Position = UDim2.new(0.5, -175, 0.5, -185)
    ...
    hdrFill.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
    ...
    cont.Position = UDim2.new(0, 0, 0, 45)
    cont.Size = UDim2.new(1, 0, 1, -45)
    cont.Parent = f

    minBtn.MouseButton1Click:Connect(function()
        SFState.Minimized = not SFState.Minimized
        cont.Visible = not SFState.Minimized
        local targetSize = SFState.Minimized and UDim2.new(0, 350, 0, 40) or UDim2.new(0, 350, 0, 370)
        SFTween:Create(f, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = targetSize}):Play()
        minBtn.Text = SFState.Minimized and "+" or "-"
    end)
    ...
    sl.Text = " Select a region & server type to begin."
    sl.TextColor3 = Color3.fromRGB(0, 255, 128)
    sl.Font = Enum.Font.Code
    sl.TextSize = 11
    sl.TextXAlignment = Enum.TextXAlignment.Left
    sl.TextYAlignment = Enum.TextYAlignment.Center
    sl.Parent = sf
    SFState.StatusLbl = sl

    -- Voice toggle
    local vBtn = Instance.new("TextButton")
    vBtn.Size = UDim2.new(1, -20, 0, 28)
    vBtn.Position = UDim2.new(0, 10, 0, 52)
    vBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    vBtn.BorderSizePixel = 0
    vBtn.Text = "Server Type: All Servers"
    vBtn.TextColor3 = Color3.fromRGB(230, 230, 230)
    vBtn.Font = Enum.Font.GothamBold
    vBtn.TextSize = 11
    vBtn.AutoButtonColor = false
    vBtn.Parent = cont
    Instance.new("UICorner", vBtn).CornerRadius = UDim.new(0, 6)
    vBtn.MouseButton1Click:Connect(function()
        if SFState.Mic == "All" then
            SFState.Mic = "Voice"
            vBtn.BackgroundColor3 = Color3.fromRGB(90, 30, 90)
            vBtn.Text = "Server Type: Voice Chat Only"
        elseif SFState.Mic == "Voice" then
            SFState.Mic = "Regular"
            vBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
            vBtn.Text = "Server Type: Regular Only"
        else
            SFState.Mic = "All"
            vBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
            vBtn.Text = "Server Type: All Servers"
    ...
    rBtn.Size = UDim2.new(1, -20, 0, 26)
    rBtn.Position = UDim2.new(0, 10, 0, 85)
    rBtn.BackgroundColor3 = Color3.fromRGB(40, 90, 140)
    rBtn.BorderSizePixel = 0
    rBtn.Text = "Refresh Region Ping & List"
    ...
    rBtn.MouseButton1Click:Connect(function()
        SFRenderRegions(searchBox.Text)
        if SFState.StatusLbl then SFState.StatusLbl.Text = " [Status] Refreshed." end
    end)
    ...
    scroll.Position = UDim2.new(0, 10, 0, 153)
    scroll.BackgroundTransparency = 1
    scroll.BorderSizePixel = 0
    scroll.CanvasSize = UDim2.new(0, 0, 0, 0)
    scroll.ScrollBarThickness = 3
    scroll.ScrollBarImageColor3 = Color3.fromRGB(90, 130, 220)
    scroll.Parent = cont
    SFState.ScrollF = scroll
    ...
    lowBtn.Size = UDim2.new(0.32, -5, 0, 26)
    lowBtn.Position = UDim2.new(0, 10, 0, 290)
    lowBtn.BackgroundColor3 = Color3.fromRGB(50, 30, 70)
    lowBtn.BorderSizePixel = 0
    lowBtn.Text = "Low"
    lowBtn.TextColor3 = Color3.new(1, 1, 1)
    lowBtn.Font = Enum.Font.GothamBold
    lowBtn.TextSize = 11
    lowBtn.AutoButtonColor = false
    lowBtn.Parent = cont
    Instance.new("UICorner", lowBtn).CornerRadius = UDim.new(0, 6)

    local highBtn = Instance.new("TextButton")
    highBtn.Size = UDim2.new(0.32, -5, 0, 26)
    highBtn.Position = UDim2.new(0.33, 5, 0, 290)
    highBtn.BackgroundColor3 = Color3.fromRGB(80, 30, 30)
    highBtn.BorderSizePixel = 0
    highBtn.Text = "High"
    highBtn.TextColor3 = Color3.new(1, 1, 1)
    highBtn.Font = Enum.Font.GothamBold
    highBtn.TextSize = 11
    highBtn.AutoButtonColor = false
    highBtn.Parent = cont
    Instance.new("UICorner", highBtn).CornerRadius = UDim.new(0, 6)

    local newBtn = Instance.new("TextButton")
    newBtn.Size = UDim2.new(0.34, -10, 0, 26)
    newBtn.Position = UDim2.new(0.66, 10, 0, 290)
    newBtn.BackgroundColor3 = Color3.fromRGB(30, 70, 80)
    ...
    lowBtn.MouseButton1Click:Connect(function()
        SFState.PopMode = "Low"
        lowBtn.BackgroundColor3 = Color3.fromRGB(100, 50, 140)
        highBtn.BackgroundColor3 = Color3.fromRGB(80, 30, 30)
        newBtn.BackgroundColor3 = Color3.fromRGB(30, 70, 80)
        if SFState.StatusLbl then SFState.StatusLbl.Text = " Mode: Low Server." end
    end)
    highBtn.MouseButton1Click:Connect(function()
        SFState.PopMode = "High"
        highBtn.BackgroundColor3 = Color3.fromRGB(140, 50, 50)
        lowBtn.BackgroundColor3 = Color3.fromRGB(50, 30, 70)
        newBtn.BackgroundColor3 = Color3.fromRGB(30, 70, 80)
        if SFState.StatusLbl then SFState.StatusLbl.Text = " Mode: High Server." end
    end)
    newBtn.MouseButton1Click:Connect(function()
        SFState.PopMode = "New"
        newBtn.BackgroundColor3 = Color3.fromRGB(50, 110, 130)
        lowBtn.BackgroundColor3 = Color3.fromRGB(50, 30, 70)
        highBtn.BackgroundColor3 = Color3.fromRGB(80, 30, 30)
        if SFState.StatusLbl then SFState.StatusLbl.Text = " Mode: New Server." end
    end)

    -- Teleport button
    local hopBtn = Instance.new("TextButton")
    hopBtn.Size = UDim2.new(1, -20, 0, 38)
    hopBtn.Position = UDim2.new(0, 10, 0, 323)
    hopBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70)
    hopBtn.BorderSizePixel = 0
    hopBtn.Text = "TELEPORT TO SELECTED REGION"
    ...
    hopBtn.MouseButton1Click:Connect(SFHop)

    SFRenderRegions("")
end

local function SFOpen()
    SFBuildUI()
    if not SFState.Frame then return end
    SFState.Frame.Visible = not SFState.Frame.Visible
end

addcmd("serverfinder", {"sf", "findserver", "fsv"}, function(args, speaker)
    SFOpen()
end)
