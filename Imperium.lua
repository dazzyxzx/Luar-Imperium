if Imperium_LOADED and not _G.Imperium_DEBUG then
	-- error("Imperium is already running!", 0)
	return
end

pcall(function() getgenv().Imperium_LOADED = true end)
...
ImperiumMouse = cloneref(Players.LocalPlayer:GetMouse())
...
Title.TextSize = 18
Title.Text = "Imperium FE v" .. currentVersion
...
PluginsHint.Text = "Download plugins from the Imperium Discord (discord.gg/78ZuWSq)"
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with Imperium,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
...
function saves()
	if writefileExploit() and readfileExploit() and jsonAttempts < 10 then
		local readSuccess, out = readfile("Imperium_FE.iy", true)
		if readSuccess then
			if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
				local success, response = pcall(function()
					...
					if vtype(json.keepImperium, "boolean") then KeepInfYield = json.keepImperium else KeepInfYield = true end
				...
				if not success then
					...
					writefile("Imperium_FE.iy", defaults, true)
					wait()
					saves()
				end
			else
				writefile("Imperium_FE.iy", defaults, true)
				wait()
				local dReadSuccess, dOut = readfile("Imperium_FE.iy", true)
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("Imperium_FE.iy", HttpService:JSONEncode(update))
	end
end
...
ImperiumMouse.KeyDown:Connect(function(Key)
	if (Key==prefix) then
...
function selectPart()
	...
	local function HighlightPart()
		if selected.Adornee ~= ImperiumMouse.Target then
			selectionBox.Adornee = ImperiumMouse.Target
		else
			selectionBox.Adornee = nil
		end
	end
	ActivateHighlight = ImperiumMouse.Move:Connect(HighlightPart)
	local function SelectPart()
		if ImperiumMouse.Target ~= nil then
			selected.Adornee = ImperiumMouse.Target
			Path.Text = getHierarchy(ImperiumMouse.Target)
		end
	end
	ClickSelect = ImperiumMouse.Button1Down:Connect(SelectPart)
end
...
CMDs[#CMDs + 1] = {NAME = 'hideimp', DESC = 'Hides the main Imperium GUI'}
CMDs[#CMDs + 1] = {NAME = 'showimp / unhideimp', DESC = 'Shows Imperium again'}
CMDs[#CMDs + 1] = {NAME = 'keepimp', DESC = 'Auto execute Imperium when you teleport through servers'}
CMDs[#CMDs + 1] = {NAME = 'unkeepimp', DESC = 'Disable keepimp'}
CMDs[#CMDs + 1] = {NAME = 'togglekeepimp', DESC = 'Toggles keepimp'}
...
function checkTT()
	local t
	local guisAtPosition = COREGUI:GetGuiObjectsAtPosition(ImperiumMouse.X, ImperiumMouse.Y)
	...
	if t ~= nil and t:GetAttribute("Title") ~= nil then
		local x = ImperiumMouse.X
		local y = ImperiumMouse.Y
		local xP
		local yP
		if ImperiumMouse.X > 200 then
			xP = x - 201
		else
			xP = x + 21
		end
		if ImperiumMouse.Y > (ImperiumMouse.ViewSizeY-96) then
			yP = y - 97
		else
			yP = y
		end
			...
			if cmd then
				...
				if infTimes then
					while lastBreakTime < cmdStartTime do
						...
						if not success and _G.Imperium_DEBUG then
...
local MousePositionToVector2 = function()
	return Vector2.new(ImperiumMouse.X, ImperiumMouse.Y)
end
...
local function clicktpFunc()
	pcall(function()
		...
		local hitPosition = ImperiumMouse.Hit.Position
...
ImperiumMouse.Button1Down:Connect(function()
	for i,v in pairs(binds) do
		...
		elseif v.COMMAND == 'clickdel' then
			...
			if input == 'RightClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
				pcall(function() ImperiumMouse.Target:Destroy() end)
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'imperium_fe.iy' or name == 'imperium_fe' then
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/EdgeImperium/infiniteyield/master/source'))()")
	end
end)
...
addcmd('keepimp', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = true
		notify('KeepImperium','Imperium will now run after you teleport')
		updatesaves()
	else
...
addcmd('unkeepimp', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		notify('KeepImperium','Imperium will no longer run after you teleport')
		updatesaves()
	else
		notify('Incompatible Exploit','Your exploit does not support this command (missing queue_on_teleport)')
	end
...
addcmd('togglekeepimp', {}, function(args, speaker)
	if queueteleport then
...
function sFLY(vfly)
	...
	flyKeyDown = UserInputService.InputBegan:Connect(function(input, processed)
		...
		if input.KeyCode == Enum.KeyCode.W then
			CONTROL.F = (vfly and vehicleflyspeed or imperiumflyspeed)
		elseif input.KeyCode == Enum.KeyCode.S then
			CONTROL.B = - (vfly and vehicleflyspeed or imperiumflyspeed)
		elseif input.KeyCode == Enum.KeyCode.A then
			CONTROL.L = - (vfly and vehicleflyspeed or imperiumflyspeed)
		elseif input.KeyCode == Enum.KeyCode.D then
			CONTROL.R = (vfly and vehicleflyspeed or imperiumflyspeed)
		elseif input.KeyCode == Enum.KeyCode.E and QEfly then
			CONTROL.Q = (vfly and vehicleflyspeed or imperiumflyspeed)*2
		elseif input.KeyCode == Enum.KeyCode.Q and QEfly then
			CONTROL.E = -(vfly and vehicleflyspeed or imperiumflyspeed)*2
		end
	...
	mfly2 = RunService.RenderStepped:Connect(function()
		...
		if speaker.Character:FindFirstChildWhichIsA("Humanoid") and root and root:FindFirstChild(velocityHandlerName) and root:FindFirstChild(gyroHandlerName) then
			...
			if direction.X > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((vfly and vehicleflyspeed or imperiumflyspeed) * 50))
			end
			if direction.X < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((vfly and vehicleflyspeed or imperiumflyspeed) * 50))
			end
			if direction.Z > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((vfly and vehicleflyspeed or imperiumflyspeed) * 50))
			end
			if direction.Z < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((vfly and vehicleflyspeed or imperiumflyspeed) * 50))
			end
...
function deleteGuisAtPos()
	pcall(function()
		local guisAtPosition = PlayerGui:GetGuiObjectsAtPosition(ImperiumMouse.X, ImperiumMouse.Y)
		for _, gui in pairs(guisAtPosition) do
...
addcmd('hideimp',{},function(args, speaker)
	isHidden = true
	...
	minimizeNum = 0
	minimizeHolder()
	if not (args[1] and tostring(args[1]) == 'nonotify') then notify('Imperium Hidden','You can press the prefix key to access the command bar') end
end)

addcmd('showimp',{'unhideimp'},function(args, speaker)
	isHidden = false
	minimizeNum = -20
	if wasStayOpen then
		maximizeHolder()
...
addcmd('hovername',{},function(args, speaker)
	...
	local function updateNameBox()
		local t
		local target = ImperiumMouse.Target
	...
	nbUpdateFunc = ImperiumMouse.Move:Connect(updateNameBox)
end)
...
addcmd("addallplugins", {"loadallplugins"}, function(args, speaker)
	...
	for _, filePath in ipairs(listfiles("")) do
		...
		if fileName and
			fileName:lower() ~= "imperium_fe.iy" and
...
ImperiumMouse.Move:Connect(checkTT)
...
task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = game:HttpGet("https://raw.githubusercontent.com/EdgeImperium/infiniteyield/master/version")
