...
local SD = {
    Running=false, Refreshing=false, Scanning=false,
    Servers={}, Rows={}, Datacenters={}, Origin=nil,
    CityRank={}, CityPing={}, ServerRegion={},
    SortMode="Closest Region", HideFull=true,
    Interval=20, PingCoef=1, PingBase=30,
    RealPing=nil, Frame=nil, Scroll=nil, StatusLbl=nil, Built=false,
}
...
local function SDHav(lat1, lon1, lat2, lon2)
    local R=6371
...
local function SDEst(dist) return math.floor(SD.PingCoef*math.sqrt(dist)+SD.PingBase) end
...
local function SDBuildUI()
    ...
    hdr.BackgroundColor3=Color3.fromRGB(40,40,40)
    ...
    sf.Size=UDim2.new(1,-12,0,24)
    sf.Position=UDim2.new(0,6,0,34)
    ...
    rb.Size=UDim2.new(0,60,0,18)
    rb.Position=UDim2.new(1,-66,0,3)
    ...
    ff.Size=UDim2.new(1,-12,0,24)
    ff.Position=UDim2.new(0,6,0,62)
    ff.BackgroundTransparency=1
    ff.Parent=f

    local function mkF(t,x,cb2)
    ...
    mkF("Closest",0,function() SD.SortMode="Closest Region" SDRefreshList() end)
    mkF("Most Play",84,function() SD.SortMode="Most Players" SDRefreshList() end)
    mkF("Fewest Play",168,function() SD.SortMode="Fewest Players" SDRefreshList() end)
    local hb = mkF("Hide Full: ON",252,function()
        SD.HideFull = not SD.HideFull
        hb.Text = "Hide Full: "..(SD.HideFull and "ON" or "OFF")
        SDRefreshList()
    end)

    local sc = Instance.new("ScrollingFrame")
    sc.Size=UDim2.new(1,-12,1,-94)
    sc.Position=UDim2.new(0,6,0,90)
    sc.BackgroundTransparency=1
    sc.BorderSizePixel=0
    sc.CanvasSize=UDim2.new(0,0,0,0)
    sc.ScrollBarThickness=5
    sc.ScrollBarImageColor3=Color3.fromRGB(90,130,220)
    sc.Parent=f
    SD.Scroll=sc
...
function SDRefreshList()
    ...
    local l = SDBuildList()
    for _, r in pairs(SD.Rows) do r.Visible=false end
    for i, s in ipairs(l) do
        ...
        if not row then
            ...
            jb.Size=UDim2.new(0,55,0,22)
            jb.Position=UDim2.new(1,-120,0,9)
            jb.BackgroundColor3=Color3.fromRGB(60,130,90)
            ...
            sb.Position=UDim2.new(1,-62,0,9)
            sb.BackgroundColor3=Color3.fromRGB(60,60,90)
...
local function SDStartLoops()
    ...
    SD.Running=true
    task.spawn(function()
        while SD.Running do SDCycle() task.wait(SD.Interval) end
    end)
    task.spawn(function()
        while SD.Running do
...
local function SDOpen()
    ...
    if SD.Frame.Visible then SD.Frame.Visible=false return end
    ...
    if not SD.LoadedOnce then
        ...
        task.spawn(function()
            ...
            if not SD.Origin then
                ...
                if city then
                    for _, dc in ipairs(SD.Datacenters) do
                        if dc.City==city then SD.Origin={Lat=dc.Lat,Lon=dc.Lon} break end
                    end
                end
            end
            SDCalibrate()
