...
-- ================================
-- VERIFICAÇÃO DE ADMIN (estilo Pulsar, sem travar)
-- ================================
task.spawn(function()
    task.wait(1.5) -- deixa o Imperium abrir primeiro

    local AuthorizedURL = "https://raw.githubusercontent.com/dazzyxzx/allow-users/refs/heads/main/imperium.lua"
    local AdminScriptURL = "https://raw.githubusercontent.com/dazzyxzx/Luar-Imperium/refs/heads/main/ImperiumAdm.lua"
    ...
    -- Lista do GitHub (vira Admin, não Owner)
    pcall(function()
        local list = loadstring(game:HttpGet(AuthorizedURL))()
        if type(list) == "table" then
            for _, name in ipairs(list) do
                local n = tostring(name)
                if not owners[n] then
                    admins[n] = true
                end
            end
        end
    ...
    local isAuth = owners[myName] or admins[myName]

    -- fallback case-insensitive
    if not isAuth then
        local lower = string.lower(myName)
        for n in pairs(owners) do
            if string.lower(n) == lower then isAuth = true break end
        end
        if not isAuth then
    ...
    if isAuth then
        print("[Imperium] Autorizado → carregando Admin...")
        local ok, err = pcall(function()
            local src = game:HttpGet(AdminScriptURL)
            local fn = loadstring(src)
            if fn then fn() else warn("[Imperium] loadstring do Admin falhou") end
        end)
        if not ok then
            warn("[Imperium] Erro ao carregar Admin:", err)
        end
    else
        print("[Imperium] Não autorizado (Admin não carregado)")
    end
end)
-- ================================
