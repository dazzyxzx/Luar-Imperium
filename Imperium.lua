...
-- ================================
-- VERIFICAÇÃO DE ADMIN (IMPERIUM) - NÃO BLOQUEIA O SCRIPT
-- ================================
task.spawn(function()
    task.wait(2) -- espera o Imperium abrir primeiro
    local ok, err = pcall(function()
        ...
        for _, name in ipairs(list) do
            if string.lower(tostring(name)) == myName then
                isAuthorized = true
                break
            end
        end
        ...
        if isAuthorized then
            print("[Imperium] Usuário autorizado → carregando Admin...")
            local src = game:HttpGet(AdminScriptURL)
            local fn = loadstring(src)
            if fn then
                fn()
            else
                warn("[Imperium] Falha no loadstring do Admin")
            end
        else
            print("[Imperium] Usuário não autorizado (Admin não carregado)")
        end
    end)
    if not ok then
        warn("[Imperium] Erro na verificação de admin:", err)
    end
end)
-- ================================
