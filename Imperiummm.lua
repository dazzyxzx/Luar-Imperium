--[[
    ...
    ║                     IMPERIUM v4.1                            ║
    ║                                                              ║
    ║  • Bolinha no CANTO SUPERIOR ESQUERDO (onde fica o Delta)    ║
    ║  • Painel no CANTO INFERIOR DIREITO                          ║
    ║  • Painel desliza pra fora da tela (igual IY)                ║
    ║  • Comandos "un" para desativar tudo                         ║
    ║  • Drag horizontal | Mobile + PC                             ║
...
local Config = {
    ...
    -- Painel: canto inferior direito (10px de margem)
    PanelX      = {1, -350},
    PanelY      = {1, -310},
    -- Bolinha: canto superior esquerdo (onde fica o Delta)
    BallX       = {0, 12},
    BallY       = {0, 12},
    PanelOpen   = false,
    ESP = {
...
--=====================================================================
-- 6. BOLINHA — canto superior esquerdo (onde fica o Delta)
--=====================================================================
...
--=====================================================================
-- 7. JANELA PRINCIPAL — canto inferior direito
--=====================================================================
...
local function ShowTooltip(cmd, btn)
    ...
    -- Como painel fica à direita, mostra tooltip à ESQUERDA do botão
    local x = abs.X - 206
...
local function StartFly()
    ...
    flyState.conn = RunService.RenderStepped:Connect(function(dt)
        ...
        local spd = UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) and flyState.turbo or flyState.speed
...
Register("fly", {"f"}, "Ativa o fly (WASD + Space/Ctrl, Shift=turbo).", function()
    if flyState.active then Notify("Fly", "Já ativo. Use ;unfly", 2) return end
    flyState.vflyMode = false
    StartFly()
    Notify("Fly", "ATIVADO — WASD | Space/Ctrl | Shift turbo", 3)
end)
Register("unfly", {}, "Desativa o fly.", function()
    if not flyState.active then Notify("Fly", "Não está ativo", 2) return end
    StopFly()
    Notify("Fly", "DESATIVADO", 2)
end)
Register("vfly", {"verticalfly"}, "Fly vertical (Space sobe, Ctrl desce).", function()
    if flyState.active then flyState.vflyMode = true Notify("VFly", "Modo vertical ON", 2) return end
    flyState.vflyMode = true
    StartFly()
    Notify("VFly", "ATIVADO — Space/Ctrl", 3)
end)
Register("flyspeed", {"fs"}, "Velocidade do fly.", function(a)
    local v = tonumber(a[1]) if v then flyState.speed = v Notify("Fly", "Speed = "..v, 2) end
end)
Register("flyturbo", {"ft"}, "Velocidade turbo do fly.", function(a)
    local v = tonumber(a[1]) if v then flyState.turbo = v Notify("Fly", "Turbo = "..v, 2) end
end)
...
Register("noclip", {"nc"}, "Atravessa paredes.", function()
    if noclipConn then Notify("Noclip", "Já ativo. Use ;unnoclip", 2) return end
...
Register("unnoclip", {"unc"}, "Desativa noclip.", function()
    if not noclipConn then Notify("Noclip", "Não está ativo", 2) return end
...
Register("infinitejump", {"ij"}, "Pulo infinito.", function()
    if ijConn then Notify("IJ", "Já ativo. Use ;unij", 2) return end
...
Register("uninfinitejump", {"unij"}, "Desativa pulo infinito.", function()
    if not ijConn then Notify("IJ", "Não está ativo", 2) return end
    ijConn:Disconnect() ijConn = nil
...
Register("fullbright", {"fb"}, "Iluminação total.", function()
    if _G.ImpFB then Notify("FB", "Já ativo. Use ;unfb", 2) return end
    _G.ImpFB = true
...
Register("unfullbright", {"unfb"}, "Desativa fullbright.", function()
    if not _G.ImpFB then Notify("FB", "Não está ativo", 2) return end
    _G.ImpFB = false
...
Register("freecam", {"fc"}, "Câmera livre.", function()
    if fcActive then Notify("Freecam", "Já ativo. Use ;unfreecam", 2) return end
    fcActive = true
...
Register("unfreecam", {}, "Desativa freecam.", function()
    if not fcActive then Notify("Freecam", "Não está ativo", 2) return end
    fcActive = false
...
Register("loopheal", {"lheal"}, "Auto-cura.", function()
    if loopHealConn then Notify("LoopHeal", "Já ativo. Use ;unloopheal", 2) return end
...
Register("unloopheal", {"unlheal"}, "Desativa auto-cura.", function()
    if not loopHealConn then Notify("LoopHeal", "Não está ativo", 2) return end
    loopHealConn:Disconnect() loopHealConn=nil
...
Register("invisible", {"invis"}, "Fica invisível (local).", function()
    ...
    if _G.ImpInvis then Notify("Invis", "Já ativo. Use ;uninvis", 2) return end
    _G.ImpInvis = true
    for _, p in ipairs(c:GetDescendants()) do
...
Register("uninvisible", {"uninvis"}, "Remove invisibilidade.", function()
    local c = LocalPlayer.Character if not c then return end
    if not _G.ImpInvis then Notify("Invis", "Não está ativo", 2) return end
    _G.ImpInvis = false
    for _, p in ipairs(c:GetDescendants()) do
...
Register("version", {"v"}, "Versão do Imperium.", function()
    Notify("Imperium","v4.1 | Open Source",3)
end)
...
--=====================================================================
-- 25. LISTA (alfabética, sem clique)
--=====================================================================
...
--=====================================================================
-- 27. TOGGLE — painel desliza pra fora
--=====================================================================
...
local function TogglePanel()
    ...
    if Config.PanelOpen then
        targetX = UDim2.new(Config.PanelX[1], Config.PanelX[2], Config.PanelY[1], Config.PanelY[2])
    else
        targetX = UDim2.new(Config.PanelX[1], Config.PanelX[2] + PANEL_SLIDE_OFFSET, Config.PanelY[1], Config.PanelY[2])
    end
...
-- 30. BOOT
--=====================================================================
-- Se começa fechado, aplica slide-out
