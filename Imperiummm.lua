--[[
    ╔══════════════════════════════════════════════════════════════╗
    ║                     IMPERIUM v5.0                            ║
    ║              UI IY Style — 70+ comandos                      ║
    ║                                                              ║
    ║  • Fly refeito (estilo IY, suave)                            ║
    ║  • Chat privado local (;chatpv)                              ║
...
--=====================================================================
-- 6. BOLINHA — canto superior esquerdo
--=====================================================================
...
local function AddChatPVMessage(author, text)
    ...
    table.insert(ChatPV.messages, {author = author, text = text})
    if #ChatPV.messages > 200 then table.remove(ChatPV.messages, 1) end

    local layout = ChatPV.scroll:FindFirstChildOfClass("UIListLayout")
    if layout then
        ChatPV.scroll.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y + 8)
    end
    -- Auto-scroll
    task.defer(function()
...
ChatPV.input.FocusLost:Connect(function(enter)
    ...
    AddChatPVMessage(LocalPlayer.DisplayName, text)
end)
...
--=====================================================================
-- 15. FLY — REFEITO (estilo IY, suave, mobile + PC)
--=====================================================================
local fly = {
    active = false,
    conn = nil,
...
local function StartFly()
    ...
    -- BodyVelocity com P alto (igual IY)
    ...
    fly.conn = RunService.RenderStepped:Connect(function(dt)
        ...
        if fly.vflyMode then
            ...
            -- Mas ainda permite WASD (adiciona horizontal)
...
--=====================================================================
-- 18. ESTADOS GLOBAIS (para comandos un)
--=====================================================================
local State = {
    noclip = false,
    infinitejump = false,
...
Register("swim", {}, "Ativa modo nado (nadar rápido).", function()
    local _, h = Utils.GetChar()
    if h then
...
Register("maxzoom", {}, "Define o zoom máximo da câmera.", function(a)
    local v = tonumber(a[1]) or 500
    LocalPlayer.CameraMaxZoomDistance = v
    Notify("Zoom", "MaxZoom = "..v, 2)
end)

Register("minzoom", {}, "Define o zoom mínimo da câmera.", function(a)
...
--=====================================================================
-- 22. COMANDOS — AÇÕES NO PLAYER
--=====================================================================
Register("fakename", {}, "Muda seu nome local (fakename <nome>).", function(a)
...
Register("headsize", {}, "Muda o tamanho da cabeça (headsize <n>).", function(a)
...
Register("punch", {}, "Faz animação de soco.", function()
    local _, h = Utils.GetChar()
    if not h then return end
    local anim = Instance.new("Animation")
...
-- 23. COMANDOS — EFEITOS
--=====================================================================
Register("burn", {}, "Coloca fogo em um player (burn <player>).", function(a)
...
Register("sparkles", {}, "Efeito sparkles em um player.", function(a)
    local p = a[1] and Utils.FindPlayer(a[1]) or LocalPlayer
...
Register("smoke", {}, "Efeito de fumaça em um player.", function(a)
    local p = a[1] and Utils.FindPlayer(a[1]) or LocalPlayer
...
--=====================================================================
-- 25. COMANDOS — ANTI/AUTO
--=====================================================================
Register("antiafk", {"aafk"}, "Anti-AFK (não kicka por inatividade).", function()
...
--=====================================================================
-- 26. COMANDOS — CHAT PV + HUD
--=====================================================================
Register("chatpv", {"cpv"}, "Abre/fecha o chat privado local.", function()
    ChatPV.open = not ChatPV.open
    ChatPV.frame.Visible = ChatPV.open
    if ChatPV.open then
        ChatPV.input:CaptureFocus()
    end
...
Register("version", {"v"}, "Versão do Imperium.", function()
    Notify("Imperium","v5.0 | Open Source",3)
end)
...
Notify("Imperium", "v5.0 — "..#CmdList.." comandos", 4)
...
_G.Imperium = {
    ...
    Logs=Logs, Waypoints=WPs, ESP=ESP, Fly=fly,
    GUI=ScreenGui, Panel=Panel, Ball=Ball, TopBar=TopBar, ChatPV=ChatPV,
    TogglePanel=TogglePanel,
    Version="5.0", IsOnMobile=IsOnMobile,
}