...
-- ================================
-- ANTI-LEAK / VERIFICAÇÃO (UNIVERSAL)
-- ================================
local function crash(msg)
   msg = msg or "Imperium | Anti-Leak detectou alteração."
   pcall(function()
...
-- Nome ofuscado: "Imperium"
local function DH()
   local bytes = {73,109,112,101,114,105,117,109} -- I m p e r i u m
   local s = ""
...
-- Dono (só este recebe tag Owner)
local owners = {
   ["Riftod37"] = true,
}

-- Admins (tag Admin, não Owner)
local admins = {
   ["CombatUser507"] = true,
   ["Ysma_303"] = true,
}

-- Lista extra via GitHub (opcional, não trava se falhar)
-- quem estiver na lista vira Admin (não Owner)
pcall(function()
   local list = loadstring(game:HttpGet("https://raw.githubusercontent.com/dazzyxzx/allow-users/refs/heads/main/imperium.lua"))()
...
if owners[LocalPlayer.Name] or LocalPlayer.Name == leaderName then
   RPName = "[ " .. HUB_NAME .. " Owner ]"
elseif admins[LocalPlayer.Name] then
   RPName = "[ " .. HUB_NAME .. " Admin ]"
end
...
-- Anti alteração direta do nome
if HUB_NAME ~= ("Imp".."eri".."um") then
   crash()
end
...
-- ================================
-- RP Name (só funciona em Brookhaven - não trava em outros jogos)
-- ================================
pcall(function()
   ...
   if re then
      ...
      if nameRemote then
         nameRemote:FireServer("RolePlayBio", RPBio)
         nameRemote:FireServer("RolePlayName", RPName)
      end
      if colorRemote then
         colorRemote:FireServer("PickingRPBioColor", Color3.new(1, 0, 0))
         colorRemote:FireServer("PickingRPNameColor", Color3.new(0, 0, 0))
      end
   end
end)
...
print("Imperium | Loaded")
print("Imperium | by Lunny + Bazuka")

-- Window
local Window = Libary:MakeWindow({
   Title = WindowTitle,
   SubTitle = "by: Lunny + Bazuka",
   SaveFolder = "imperium"
})
...
InfoTab:AddParagraph({ "Owner", "Lunny + Bazuka" })
InfoTab:AddParagraph({ "Programmer", "KitK4t Studios + Lunny" })
...
task.spawn(function()
   ...
   local function crash2()
      if LocalPlayer then
        LocalPlayer:Kick("Imperium | Falha na verificação de integridade.")
      end

      while true do end
   end

   -- Recria o nome original escondido (Imperium)
   local function DH_CHECK()
      local t = {73,109,112,101,114,105,117,109} -- Imperium
      local s = ""
...
-- o script mais protegido do mundo e o Imperium!
...
local function applyToAllCars(valueType, value)
   ...
   newNotify("Imperium", "Imperium " .. carsModified .. " cars modified")
end
...
CarTab:AddTextBox({
   ...
   Callback = function(text)
      ...
      if num then
        speedValue = num
        newNotify("Imperium", "Speed set to: " .. num)
      else
        newNotify("Imperium", "Invalid number")
      end
   end
})
...
CarTab:AddTextBox({
   ...
   Callback = function(text)
      ...
      if num then
        turboValue = num
        newNotify("Imperium", "Turbo set to: " .. num)
      else
        newNotify("Imperium", "Invalid number")
      end
   end
...
print("Imperium Admin | Discord: https://discord.gg/NnkWr2M5rz")
print("Imperium https://discord.gg/NnkWr2M5rz")
